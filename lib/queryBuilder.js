    ...
    patchQuery: function() {
        ...
        this._.each( this.body, function( value, key ) {
            ...
            if( this.request.headers.iosapp && dbKey === 'birthdate' ) { 
                setArray.push(this.format('%s = (select to_timestamp($%s)::date)', dbKey, ctr++))
            } else {
                setArray.push(this.format('%s = $%s', dbKey, ctr++))
            }
            values.push( value )
        }, this )
        ...
        return {
            query: this.format( 'UPDATE %s SET %s WHERE id = $1%s %s;', this.name, setArray.join(', '), contextFilter, returning ),
            values: values
        }
    },
