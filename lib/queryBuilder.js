module.exports = Object.create( {
    ...
    deleteQuery: function() {
        return { query: `DELETE FROM "${this.path[1]}" WHERE id = $1`, values: [ this.path[2] ] }
    },
    ...
    getQuery: function() {
        var selectArray = ( this.columns || this.tables[ this.path[1] ].columns ).map( column => {
            return column.name === 'location'
                ? `ST_AsGeoJSON(location) as location`
                : this.format( '"%s"."%s"', this.path[1], column.name )
        } )
